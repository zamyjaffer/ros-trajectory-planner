...
- This is a ROS project that creates a trajectory planner for an end-effector for a robots movement. This trajectory planner consists of 4 nodes:
  1. 'points_generator' which generates random pramaters for the cubic trajectory.
  2. 'compute_cubic_coeffs' which recieves the parameters from the first node and calculates foefficients for the trajectory.
  3. 'cubic_traj_planner' which subscribes to the coefficients from the second node and generates the trajectory
  4. 'plot_cubic_traj' which subsribes to the trajectory and plots the acceleration, position and velocity trajectories
